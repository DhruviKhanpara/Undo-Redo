...
public class undo_redo extends JFrame implements ActionListener, ItemListener{
    ...
    String f="";  //for store file name at saving the file
    String texts="";  //initial text inside textarea
    ...
    public undo_redo() {  
        a = new JTextArea(20, 55); // textarea (rows,columns)
        a.setFont(a.getFont().deriveFont(size));  // initially set font size to 12.0pt
        ...
        Dimension bDim = new Dimension(60, 30);  //dimension (width,height)
        ...
        b9.setPreferredSize(bDim);
        b9.setMaximumSize(bDim);
        b10.setPreferredSize(bDim);
        b10.setMaximumSize(bDim);
        b11.setPreferredSize(bDim);
        b11.setMaximumSize(bDim);
...
//                    m.undo();  
                	UNDO(Undo,Redo);
                	a.setText(texts);
                	if(!Undo.isEmpty()) a.append(Undo.peek());  //set changed text to textarea
    				else a.append("");
                } catch (Exception ex) {}  
            }
        });
...
//                    m.redo();
        ...
        b8.addActionListener(new ActionListener() {  
            public void actionPerformed(ActionEvent e) {
            	if(b8.getText()=="Dark theme") {   //Check you want to move to dark theme
            ...
            public void actionPerformed(ActionEvent e) { 
            	if(getTitle().equals("Untitled")) {
                    ...
                    if(fileChooser.showSaveDialog(null) == JFileChooser.APPROVE_OPTION){
                    	f = fileChooser.getSelectedFile().toString().replace("\\", "\\\\");
        ...
        b11.addActionListener(new ActionListener() {  
            public void actionPerformed(ActionEvent e) { 
                ...
                if(result == chooser.APPROVE_OPTION){
                	f = chooser.getSelectedFile().toString().replace("\\", "\\\\");
                	try {
                		String msg="";
                		File file = new File(f);
                	...
                	System.out.println("Open command cancled");
                }
            }  
        });
...
//                        m.redo();  
        	...
        	public void keyReleased(KeyEvent e) {
            	...
            	if(l.contains(e.getKeyChar()) ||
            			Character.isLetterOrDigit(e.getKeyChar()) ||
            			e.getKeyCode() == KeyEvent.VK_BACK_SPACE ||
            			e.getKeyCode() == KeyEvent.VK_DELETE ||
            			(e.getKeyCode()==KeyEvent.VK_V) && ((e.getModifiers() & KeyEvent.CTRL_MASK)!=0) ||
            			(e.getKeyCode()==KeyEvent.VK_X) && ((e.getModifiers() & KeyEvent.CTRL_MASK)!=0)) {
            		WRITE(Undo,a.getText().substring(texts.length()));
